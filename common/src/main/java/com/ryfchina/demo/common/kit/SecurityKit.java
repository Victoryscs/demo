...
public class SecurityKit {
    ...
    final static Logger logger = Logger.getLogger(SecurityKit.class);
    ...
    public static String generateRSA(String key, String data, int mode, int type) {
        try {
            ...
            Key strkey = null;
            if (mode != type) { // 生成公钥
                KeySpec keySpec = new X509EncodedKeySpec(SecurityKit
                        .str2bytes(key)); // X.509编码
                strkey = keyFactory.generatePublic(keySpec);
            } else { // 生成私钥
                KeySpec keySpec = new PKCS8EncodedKeySpec(SecurityKit
    ...
    public static boolean verifySHA1withRSA(String key, String plaindata,
        ...
        try {
            KeyFactory keyFactory = KeyFactory.getInstance("RSA");
            KeySpec keySpec = new X509EncodedKeySpec(SecurityKit.str2bytes(key)); // X.509编码
    ...
    public static String generateTransMac(String randomNum,String keyMain,String command,byte[] timeMillis,String keyMac)
    {
        ...
        try{
            ...
            String processKey=SecurityKit.desecb(keyMain,randomNum,0);//过程密钥
            logger.debug("获得过程密钥"+processKey);
            String mdata;   //加密后的密文
            String used_data;  //N字节要发送的数据信息
            if(appdata!=null)
                used_data=appdata+SecurityKit.bytesToHexString(timeMillis);
            else{
                used_data=SecurityKit.bytesToHexString(timeMillis);
            }

//     if(used_data.length()%16!=0){
            //用过程密钥Key对“N字节要发送的数据信息+8字节的时间戳”进行3DES算法加密(填充80)

            logger.debug("用过程密钥加密数据："+SecurityKit.padding80(used_data));
            mdata=SecurityKit.desecb(processKey,SecurityKit.padding80(used_data),0);//过程密钥加密数据
            logger.debug("获得密文："+mdata);
//    	}else{
//    		//用过程密钥Key对“N字节要发送的数据信息+8字节的时间戳”进行3DES算法加密(填充80)
//    		   	logger.debug("用过程密钥加密数据："+used_data);	
//    		   	mdata=SecurityKit.desecb(processKey,used_data,0);//过程密钥加密数据
//    		   	logger.debug("获得密文："+mdata);
//    	}	

            //随机数生成MAC过程密钥  
            logger.debug("生成MAC过程密钥的随机数："+randomNum);
            String processKey_mac=SecurityKit.desecb(keyMac,randomNum,0);
            ...
            if(MAC_data.length()%16!=0){
                logger.debug("用mac过程密钥加密数据："+SecurityKit.padding80(MAC_data));
                mac=SecurityKit.generateMAC(processKey_mac,SecurityKit.padding80(MAC_data),ICV,2,1);
                logger.debug("获得4字节mac："+mac);
            }else{
                logger.debug("用mac过程密钥加密数据："+MAC_data);
                mac=SecurityKit.generateMAC(processKey_mac,MAC_data,ICV,2,1);
                logger.debug("获得4字节mac："+mac);
            }
...
//    	}	
            ...
            if (command.length()>=256) {
                ...
                byte[] commandHead = SecurityKit.str2bytes(command_head);
                System.arraycopy(len, 0, commandHead, 4, 2);
                command_head = SecurityKit.bytesToHexString(commandHead);
            } else {
                byte[] len = new byte[1];
    ...
    public static byte[] MakeCommand_SE(String code_value , byte code_tag, String keyMain)
    {
        ...
        String random=StringKit.randomStr2Ascii(16);
        byte[] random_byte=SecurityKit.str2bytes(random);
        logger.debug("生成随机数："+random);

        //用主密匙keyMask使用3DES算法对16字节的随机数进行加密
        logger.debug("用主密钥加密随机数："+random);
        String processKey=SecurityKit.desecb(keyMain,random,0);//过程密钥
        logger.debug("获得过程密钥"+processKey);

        //用过程密钥processKey使用3DES算法对16字节的新密钥进行加密
        logger.debug("用过程密钥加密新密钥："+code_value);
        String code_data=SecurityKit.desecb(processKey,code_value,0);//密文
        byte[] code=SecurityKit.str2bytes(code_data);
    ...
    public static boolean volidMac(String appData,String keyMac){
        ...
        try{
            ...
            String processKey=SecurityKit.desecb(keyMac,randomNum,0);
            ...
            String mac;
            if(MAC_data.length()%16!=0){
                logger.debug("用MAC密钥计算数据："+SecurityKit.padding80(MAC_data));
                mac=SecurityKit.generateMAC(processKey,SecurityKit.padding80(MAC_data),ICV,2,1);
                logger.debug("返回数据的mac值为："+mac);
            }else{
                logger.debug("用MAC密钥计算数据："+MAC_data);
                mac=SecurityKit.generateMAC(processKey,MAC_data,ICV,2,1);
                logger.debug("返回数据的mac值为："+mac);
            }
    ...
    public static String desData(String appData,String keyMain){
        try{
            ...
            String randomNum=appData.substring(appData.length()-40, appData.length()-8);//得到16位随机数密文
            logger.debug("返回数据的随机数为："+randomNum);
            String processKey=SecurityKit.desecb(keyMain,randomNum,0);//得到解密的过程密钥
            logger.debug("随机数生成过程密钥为："+processKey);

            logger.debug("用过程密钥解密密文数据："+appData.substring(0, appData.length()-44));
            String redata=SecurityKit.desecb(processKey,appData.substring(0, appData.length()-44),1);
    ...
    /**
     * 鐟欙絽鐦戦弫鐗堝祦
     * @param actionInfo actionInfo鐎靛棙鏋?
     * @return 閺勫孩鏋?
    ...
    public static String decode(String actionInfo) {
        ...
        // 閼惧嘲褰囨潻鍥┾柤鐎靛棝鎸?
        String processKey = SecurityKit.desecb(key,randData,0);
        // 鐟欙絽鐦憇ingData
        String actionInfoString = SecurityKit.desecb(processKey, singData, 1);
        // 鐏?6鏉╂稑鍩楅弫鏉跨摟鐟欙絿鐖滈幋鎰摟缁楋缚瑕?闁倻鏁ゆ禍搴㈠閺堝鐡х粭锔肩礄閸栧懏瀚稉顓熸瀮閿?
        try {
            actionInfoString = SecurityKit.hexStringToString(actionInfoString);
...
//				System.out.println("鐎瑰本鍨氱憴锝呯槕閿涘son\n"+actionInfoString);
    ...
    /**
     * 閸旂姴鐦戦弫鐗堝祦
     * @param actionInfo actionInfo閺勫孩鏋?
     * @return actionInfo鐎靛棙鏋?
     * @throws UnsupportedEncodingException
     */
    public static String encode(String actionInfo) {
        // 閻㈢喐鍨氶梾蹇旀簚閺?
        String randData = StringKit.generalStringToAscii(8)+StringKit.generalStringToAscii(8);
        // 閼惧嘲褰囨潻鍥┾柤鐎靛棝鎸?
        String processKey = SecurityKit.desecb(key, randData, 0);
        // 鐏忓摳ctionInfo鏉烆剚宕?6鏉╂稑鍩楅崥搴礉鐞?0
        try {
            actionInfo = SecurityKit.padding80(SecurityKit.bytesToHexString(actionInfo.getBytes("UTF-8")));
        } catch (UnsupportedEncodingException e) {
            // TODO Auto-generated catch block
            logger.error(e);
        }
        // 鐏忓棗鐡х粭锔胯缂傛牜鐖滈幋?6鏉╂稑鍩楅弫鏉跨摟,闁倻鏁ゆ禍搴㈠閺堝鐡х粭锔肩礄閸栧懏瀚稉顓熸瀮閿?
        actionInfo = SecurityKit.encodeHexString(actionInfo);
        // 閸旂姴鐦?
        actionInfo = SecurityKit.desecb(processKey,actionInfo, 0);
        // 閺?绮撻悽鐔稿灇鐎靛棙鏋?
...
//		System.out.println("鐎瑰本鍨氶崝鐘茬槕閿涘son\n"+actionInfo);
    ...
    /**
     * 将byte数组转换成16进制组成的字符串(大写) 例如 一个byte数组 b[0]=0x07;b[1]=0x10;...b[5]=0xFB;
     * byte2hex(b); 将返回一个字符串"0710BE8716FB"
     *
     * @param bytes 待转换的byte数组
     * @return
     */
    public static String bytesToHexStringNew(byte[] bytes)
    {
        if (bytes == null) {
            return "";
        }
    ...
    public static void main(String[] args){
        String va = SecurityKit.desecb("A94129EB6A6B77AFE6ECD5B8E42EB053", "505152535455565758595A5B5C5D5E5F", 0);
//    	String pw=SecurityKit.desData("4B9CE3149E91AD32ED82F2D31DCEDE4F52565648485752535349565556575154986FC3D6", "1234434556689886");
...
//        // RSA信息
//        String n = ""; // N
//        // RSA的模modulus
//        String e = "65537"; // E RSA公钥的指数exponent
//        String d = ""; // D
//        // RSA私钥的指数exponent
//        String rsaPrivateKey = ""; // RSA私钥
//        // 用PKCS#8编码
//        String rsaPublicKey = ""; // RSA公钥
//        // 用X.509编码
//        // SCP信息
...
//        String loadfile = "01001ADECAFFED010204000110A0000000770002FFFFFFFF890000000002001F001A001F00140014005E001A017C00210037000000EE000400020"
...
//        String packetMT = "80E602001B10D2760000050002FFFFFFFF8900000202000006EF04C602000000";
//        String bigpacketMT = "80E602006C10D2760000050002FFFFFFFF8900000202000006EF04C60200000010D2760000050002FFFFFFFF8900000202000006EF04C60200000010D2760000050002FFFFFFFF8900000202000006EF04C60200000010D2760000050002FFFFFFFF8900000202000006EF04C602000000";
...
////        KeySessionInfo sessionInfo = new KeySessionInfo(); // 保存主机随机数，以便后续命令使用。
...
//                .println("----------------------------------------------计算SCP02卡片密文----------------------------------------------");
//        // 计算SCP02卡片密文
//        String derivationstr = "0182" + "1135" + "000000000000000000000000";// 过程密钥常量0182
...
////        KeySessionInfo sessionInfo1 = new KeySessionInfo(); // 保存主机随机数，以便后续命令使用。
...
////        Response0348 response0348 = analyse0348ResponsePacket(2, spi, kic, kid,
////                packetMO);
