...
public final class UuidKit {
	...
	public static synchronized String getLongUNID() {
		...
		return (new StringBuilder(String.valueOf(currentTime))).append(longFormat.format(longSequence)).toString();
	}
	 ...
	 */
	/*
	 * public static synchronized String getSmallUNID() { String currentTime = getCurrentDateString("yyMMddHHmmss"); if
	 * (smallTime == null || smallTime.compareTo(currentTime) != 0) { smallTime = currentTime; smallSequence = 1L; }
	 * else { smallSequence++; } return (new
	 * StringBuilder(String.valueOf(currentTime))).append(smallFormat.format(smallSequence)).toString(); }
	 */
